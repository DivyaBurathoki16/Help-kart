  ...
  const fetchChats = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setInquiries(conversations.filter((c) => c.type === 'INQUIRY'));
        
        // Filter booking chats and remove duplicates based on bookingId
        const bookingChatsFiltered = conversations
          .filter((c) => c.type === 'BOOKING' && c.bookingId)
          .reduce((acc, current) => {
            // Check if we already have a chat for this bookingId
            const existing = acc.find((item) => 
              item.bookingId?._id?.toString() === current.bookingId?._id?.toString() ||
              item.bookingId?.toString() === current.bookingId?.toString()
            );
            // Keep the one with the most recent lastMessageAt or the one with unread messages
            if (!existing) {
              acc.push(current);
            } else if (
        ...
        setBookingChats(bookingChatsFiltered);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#030712] text-slate-900 dark:text-white py-12 transition-colors duration-500 overflow-x-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-16">
          {/* Service Inquiries */}
          <Card className="p-10 bg-white/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-3xl rounded-[2.5rem] shadow-xl dark:shadow-2xl">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Pre-booking chats from customers
                </p>
              </div>
              <div className="p-3 bg-blue-600/10 dark:bg-blue-600/20 rounded-2xl text-blue-600 dark:text-blue-400">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {inquiries.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-slate-500 dark:text-slate-400">No service inquiries yet</p>
              </div>
            ) : (
                  ...
                  <button
                    ...
                    onClick={() =>
                      setChatModal({
                        ...
                        type: 'INQUIRY',
                        contextInfo: {
                          serviceTitle: inquiry.serviceId?.title || 'Service Inquiry',
                        },
                      })
                    }
                    className="w-full p-4 bg-slate-50 dark:bg-neutral-800/50 rounded-xl border border-slate-200 dark:border-neutral-700 hover:bg-slate-100 dark:hover:bg-neutral-800 transition-all text-left group"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-900 dark:text-neutral-100 truncate">
                          {inquiry.serviceId?.title || 'Service Inquiry'}
                        </p>
                        <p className="text-xs text-slate-500 dark:text-neutral-400 mt-1">
                          {inquiry.clientId?.name || 'Customer'}
                        </p>
                      </div>
          ...
          <Card className="p-10 bg-white/80 dark:bg-white/5 border border-slate-200 dark:border-white/10 backdrop-blur-3xl rounded-[2.5rem] shadow-xl dark:shadow-2xl">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Active conversations for bookings
                </p>
              </div>
              <div className="p-3 bg-amber-600/10 dark:bg-amber-600/20 rounded-2xl text-amber-600 dark:text-amber-400">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
              </div>
            ...
            {bookingChats.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-slate-500 dark:text-slate-400">No active booking chats</p>
              </div>
            ) : (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {bookingChats.map((chat) => (
                  <button
                    ...
                    onClick={() =>
                      setChatModal({
                        ...
                        type: 'BOOKING',
                        contextInfo: {
                          bookingTitle: chat.bookingId?.service?.title || 'Booking Chat',
                        },
                      })
                    }
                    className="w-full p-4 bg-slate-50 dark:bg-neutral-800/50 rounded-xl border border-slate-200 dark:border-neutral-700 hover:bg-slate-100 dark:hover:bg-neutral-800 transition-all text-left group"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-900 dark:text-neutral-100 truncate">
                          {chat.bookingId?.service?.title || 'Booking Chat'}
                        </p>
                        <p className="text-xs text-slate-500 dark:text-neutral-400 mt-1">
                          {chat.clientId?.name || 'Customer'} •{' '}
                          {chat.bookingId?.bookingDate
                            ? new Date(chat.bookingId.bookingDate).toLocaleDateString()
                            : ''}
                        </p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <svg
                          className="w-5 h-5 text-slate-400 group-hover:text-amber-600 transition-colors"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
        ...
        {chatModal && (
          <ChatModal
            ...
            type={chatModal.type}
            contextInfo={chatModal.contextInfo}
          />
        )}
      </div>
    </div>
